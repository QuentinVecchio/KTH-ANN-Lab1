...
def scenario3_1_3bis():
    ...
    multipleLayerNN = NN2.MultipleLayer(batch_size=100, nb_eboch=100, lr=0.001, hidden_layer_size= 5)
...
def scenario3_2_1():
    ...
    Tbis = np.reshape(T,(len(T),1))
    multipleLayerNN = NN2.MultipleLayer(batch_size=-1, nb_eboch=100, lr=0.001, hidden_layer_size= 2)
    WHistoryMNN, eHistoryMNN = multipleLayerNN.fit(X, Tbis)
    graph.plotNNInformations("Multiple Layer NN", X, T, WHistoryMNN[-1], eHistoryMNN)
    graph.plotDecisionBoundaryAnim("Multiple Layer NN Anim", X, T, WHistoryMNN)
...
def scenario3_3_3():
    ...
    n1= 5
    x, y = np.mgrid[-5.0:5.0:10j, -5.0:5.0:20j]  # j
    X = np.column_stack([x.flat, y.flat])
    Y = f(X)

    multipleLayerNN = NN2.MultipleLayerGN(batch_size=-1, nb_eboch=1000, lr=0.001, hidden_layer_size=5)
    WHistory, eHistory, pHistory = multipleLayerNN.fit(X.T, Y.T,n1)
